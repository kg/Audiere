 ...
 * --
 * portable typedefs
 * you'll probably want to change the names
 */
#ifdef _MSC_VER  /* VC++ */

typedef signed   __int64 mad_s64_t;
typedef unsigned __int64 mad_u64_t;

#else            /* assume gcc */

typedef signed   long long mad_s64_t;
typedef unsigned long long mad_u64_t;
...
#  else
#   define mad_f_mul(x, y)  \
    ((mad_fixed_t) (((mad_s64_t) (x) * (y)) >> MAD_F_SCALEBITS))
#  endif
...
#  ifdef _MSC_VER
    ...
    __inline mad_f_mul(mad_fixed_t x, mad_fixed_t y)
    {
      mad_fixed64hi_t __hi;
      mad_fixed64lo_t __lo;
      MAD_F_MLX(__hi, __lo, x, y);
