...
static
struct fixedfloat {
  unsigned mantissa  : 27;  /* 2001.04.30 - Chad Austin, bit-fields should be of int type */
  unsigned exponent  : 5;
} const rq_table[8207] = {
  ...
  if (!((mode_extension & 0x1) && gr1ch)) {
    ...
    for (part = 0; part < 4; ++part) {
      ...
      for (i = 0; i < nsfb[part]; ++i) {
        ...
        channel->scalefac[n] = (unsigned char)is_pos;
        gr1ch->scalefac[n++] = (unsigned char)(is_pos == max);
      }
    }
                          ...
                          unsigned char const *sfbwidth)
{
  ...
  for (i = 0; i < 39; ++i)
    modes[i] = (short)header->mode_extension;
  ...
  if (header->mode_extension & i_stereo) {
    ...
    if (right_ch->block_type == 2) {
      ...
      for (i = 0; i < lower; ++i)
        modes[i] = (short)(header->mode_extension & ~i_stereo);
...
int mad_layer_III(struct mad_stream *stream, struct mad_frame *frame)
{
  ...
  if (si.main_data_begin == 0) {
    ...
    // initialize to empty (so compiler doesn't complain)
    ptr.byte  = NULL;
    ptr.cache = 0;
    ptr.left  = 0;
