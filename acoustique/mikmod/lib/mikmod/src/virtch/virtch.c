...
// =====================================================================================
    static ULONG samples2bytes(VIRTCH *vc, ULONG samples)
// =====================================================================================
{
...
// =====================================================================================
    static ULONG bytes2samples(VIRTCH *vc, ULONG bytes)
// =====================================================================================
{
        ...
        {   // Check if the mdriver has asked for a preemption into the MD_Player
            ...
            {   VINFO *vnf;
                ...
                {   uint   tpor, mddiv = RAMPLEN_NOTECUT;
                    ...
                    {
                        if(vnf->handle) // && vnf->handle->mixer)
                        {   // always ramp to 0 (special coded where vnf->lvol and vnf->rvol
                            ...
                            if(!vnf->volramp) vnf->handle = NULL;
                        }
                    }
                ...
                for(t=0; t<vc->numchn; t++, vnf++)
                {
                    ...
                    {   vnf->current = (INT64S)(vnf->start) << FRACBITS;
                        ...
                        if(vc->mode & DMODE_NOCLICK)
                        {
                            ...
                            if(vnf->handle)
                            {   if(vnf->handle->format & SF_16BITS)
                                {
                                    uint  i;
                                    SWORD *src = (SWORD *)vnf->handle->data;
                                    src += vnf->start;
                    ...
                    if(vnf->frq && vnf->handle) // && vnf->handle->mixer)
...
// =====================================================================================
{
    return(md->vc->vinf[voice].handle == NULL);
}
